/*
26 5,13 * * * jd_newfarmlottery_m.js
*/
const $ = new Env('新农场幸运转盘_m');
...
!(async () => {
  ...
  for (let _0x100a21 = 0; _0x100a21 < bdy_0x213fd6.length; _0x100a21++) {
    if (bdy_0x213fd6[_0x100a21]) {
      bdy_0x3147e0 = bdy_0x213fd6[_0x100a21];
      $.UserName = decodeURIComponent(bdy_0x3147e0.match(/pt_pin=([^; ]+)(?=;?)/) && bdy_0x3147e0.match(/pt_pin=([^; ]+)(?=;?)/)[1]);
      $.index = _0x100a21 + 1;
      $.isLogin = true;
      $.nickName = "";
      $.farmInfo = "";
...
async function bdy_0x1378f9() {
  ...
  if ($.initForTurntableFarmRes && $.initForTurntableFarmRes.code === 0) {
    ...
    if ($.initForTurntableFarmRes.data.lotteryChances > 0) {
      ...
      console.log("开始抽奖...");
      let _0x2c8572 = "";
      for (let _0x48825a = 0; _0x48825a < $.initForTurntableFarmRes.data.lotteryChances; _0x48825a++) {
        await bdy_0x40e4c5();
        console.log("第" + (_0x48825a + 1) + "次抽奖");
  ...
  if ($.farmAssistResult.code === 0) {
    if ($.farmAssistResult.data.result.assistFriendList && $.farmAssistResult.data.result.assistFriendList.length >= 2) {
      if ($.farmAssistResult.data.result.status === 2) {
        ...
        for (let _0x4df6c7 of Object.keys($.farmAssistResult.data.result.assistStageList)) {
          ...
          if (_0x664ffb.stageStaus === 2) {
            await bdy_0x2267f5();
            await $.wait(500);
            if ($.receiveStageEnergy.code === 0) {
              console.log("成功领取第" + (Number(_0x4df6c7) + 1) + "段助力奖励：" + $.receiveStageEnergy.data.result.amount + "g💧");
        ...
        $.farmAssistResult.data.result.status === 3 && (console.log("已经领取过好友助力额外奖励"), bdy_0x456112 += "【额外奖励】已领取过\n");
      }
    } else {
    ...
    if ($.farmAssistResult.data.result.assistFriendList && $.farmAssistResult.data.result.assistFriendList.length > 0) {
      ...
      $.farmAssistResult.data.result.assistFriendList.map((_0x19605b, _0x5d5aaf) => {
        _0x5d5aaf === $.farmAssistResult.data.result.assistFriendList.length - 1 ? _0x331416 += _0x19605b.nickname || "匿名用户" : _0x331416 += (_0x19605b.nickname || "匿名用户") + ",";
        let _0x5e9411 = new Date(_0x19605b.time),
          _0x4fe80c = _0x5e9411.getFullYear() + "/" + ("0" + (_0x5e9411.getMonth() + 1)).slice(-2) + "/" + ("0" + _0x5e9411.getDate()).slice(-2) + " " + ("0" + _0x5e9411.getHours()).slice(-2) + ":" + ("0" + _0x5e9411.getMinutes()).slice(-2) + ":" + ("0" + _0x5e9411.getSeconds()).slice(-2);
    ...
    if ($.masterHelpResult.code === "0") {
      $.masterHelpResult.masterHelpPeoples && $.masterHelpResult.masterHelpPeoples.length >= 5 ? !$.masterHelpResult.masterGotFinal ? (await bdy_0x3426fd(), $.masterGotFinished.code === "0" && (console.log("已成功领取好友助力奖励：【" + $.masterGotFinished.amount + "】g💧"), bdy_0x456112 += "【额外奖励】" + $.masterGotFinished.amount + "g💧领取成功\n")) : (console.log("已经领取过5好友助力额外奖励"), bdy_0x456112 += "【额外奖励】已被领取过\n") : (console.log("助力好友未达到5个"), bdy_0x456112 += "【额外奖励】领取失败,原因：给您助力的人未达5个\n");
      if ($.masterHelpResult.masterHelpPeoples && $.masterHelpResult.masterHelpPeoples.length > 0) {
        let _0x14e112 = "";
        $.masterHelpResult.masterHelpPeoples.map((_0x137d0b, _0x5b0c57) => {
          _0x5b0c57 === $.masterHelpResult.masterHelpPeoples.length - 1 ? _0x14e112 += _0x137d0b.nickName || "匿名用户" : _0x14e112 += (_0x137d0b.nickName || "匿名用户") + ",";
          let _0x7abb06 = new Date(_0x137d0b.time),
            _0xa493fa = _0x7abb06.getFullYear() + "-" + (_0x7abb06.getMonth() + 1) + "-" + _0x7abb06.getDate() + " " + _0x7abb06.getHours() + ":" + _0x7abb06.getMinutes() + ":" + _0x7abb06.getMinutes();
...
async function bdy_0x45d553() {
  ...
  if (_0x359f06) {
    ...
    _0x359f06 && (console.log("开始水滴雨任务,这是第" + ($.farmTask.waterRainInit.winTimes + 1) + "次，剩余" + (2 - ($.farmTask.waterRainInit.winTimes + 1)) + "次"), await bdy_0x999292(), console.log("水滴雨waterRain"), $.waterRain.code === "0" && (console.log("水滴雨任务执行成功，获得水滴：" + $.waterRain.addEnergy + "g💧"), console.log("【第" + ($.farmTask.waterRainInit.winTimes + 1) + "次水滴雨】获得" + $.waterRain.addEnergy + "g💧\n")));
  } else {
  ...
  if ($.clockInInit.code === 0) {
    if ($.clockInInit.data.signInFlag == 0) {
      ...
      if ($.clockInForFarmRes.code === 0) {
        ...
        $.clockInForFarmRes.code === 210000 ? (console.log("签到失败：" + JSON.stringify($.clockInForFarmRes)), $.kuwei = true) : console.log("签到失败：" + JSON.stringify($.clockInForFarmRes));
      }
    }
  }
}
  ...
  if (_0x81f3c2 < _0xd219d2) {
    ...
    if ($.friendList.friends && $.friendList.friends.length > 0) {
      $.friendList.friends.map((_0xe5560b, _0x2cc108) => {
        _0xe5560b.friendState === 1 && _0x52a120.length < _0xd219d2 - _0x81f3c2 && _0x52a120.push(_0xe5560b.shareCode);
      });
      _0x52a120.length > 0 && console.log("需要浇水的好友shareCodes:" + JSON.stringify(_0x52a120));
      _0x52a120.length == 0 && console.log("没有需要浇水的好友!\n");
      let _0x4f12f8 = 0,
        _0x7e7117 = "";
      for (let _0x1340d5 = 0; _0x1340d5 < _0x52a120.length; _0x1340d5++) {
        await bdy_0x451ff5(_0x52a120[_0x1340d5]);
        console.log("为第" + (_0x1340d5 + 1) + "个好友浇水");
      ...
      _0x4f12f8 > 0 && console.log("已给" + _0x4f12f8 + "个好友浇水,消耗" + _0x4f12f8 * 10 + "g水\n");
      if (_0x7e7117 && _0x7e7117.length > 0) {
        console.log("【好友浇水奖励】" + _0x7e7117.substr(0, _0x7e7117.length - 1) + "\n");
      }
    } else {
...
async function bdy_0x41d620() {
  for (let _0x281169 = 0; _0x281169 < 10; _0x281169++) {
    const _0x20f23e = {
      type: 2,
      version: 24,
      channel: 1,
...
async function bdy_0x3ea445(_0x445214) {
  ...
  $.browserForTurntableFarm2Res = await request("browserForTurntableFarm", _0x2958d2);
}
...
async function bdy_0x999292() {
  ...
  $.waterRain = await request("waterRainForFarm", _0x1c798d);
}
...
async function bdy_0x370458(_0xff8604, _0x1d08c4, _0x295a5c) {
  ...
  if (_0x1d08c4 === "theme") {
    if (_0x295a5c === "1") {
      $.themeStep1 = await request(_0x181da7, _0x3f4a37);
    } else {
      _0x295a5c === "2" && ($.themeStep2 = await request(_0x181da7, _0x3f4a37));
    }
  } else {
    if (_0x1d08c4 === "venderCoupon") {
      if (_0x295a5c === "1") {
        $.venderCouponStep1 = await request(_0x181da7, _0x3f4a37);
      } else {
        _0x295a5c === "2" && ($.venderCouponStep2 = await request(_0x181da7, _0x3f4a37));
      }
    }
  }
}
...
async function bdy_0x451ff5(_0x152e94) {
  ...
  $.waterFriendForFarmRes = await request("waterFriendForFarm", _0x53028f);
}
...
function bdy_0x206190() {
  return new Promise(_0x9d9297 => {
    ...
    $.get(_0x338735, (_0x46bb97, _0x2aff81, _0x3285cb) => {
      _0x9d9297();
    });
  });
}
function bdy_0x78197e() {
  return new Promise(_0x1d241c => {
    ...
    $.get(_0xeb52df, (_0x2842fd, _0x5eeb33, _0x4e4270) => {
      try {
        if (_0x4e4270) {
...
async function bdy_0x3a2735(_0x7d53fc, _0xb24c87 = {}, _0xbf09cb = 1500) {
  $.reqnum++;
  if (bdy_0x3f17ce[_0x7d53fc]) {
    if (_0x7d53fc == "wheelsHome" && 0) {
      let _0x18d5ae = {
        appId: bdy_0x3f17ce[_0x7d53fc],
        fn: _0x7d53fc,
...
async function bdy_0x193a85() {
  ...
  return new Promise(_0x588b82 => {
    $.post(_0x146a1c, (_0x2ea1ac, _0x3cc42f, _0x5ab60a) => {
      try {
        if (_0x2ea1ac) {
          console.log("\n东东农场: API请求失败 ‼️‼️");
          ...
          bdy_0x2de21f(_0x5ab60a) && (_0x5ab60a = JSON.parse(_0x5ab60a), _0x5ab60a.code == 200 && ($.aid = _0x5ab60a.data.ext.uuid));
        }
      } catch (_0x2057ed) {
...
async function bdy_0x23535c() {
  ...
  return new Promise(_0x4728c9 => {
    $.post(_0x2f988a, (_0xfe5dc8, _0x17270f, _0x442e0f) => {
      try {
        if (_0xfe5dc8) {
          console.log("\n东东农场: API请求失败 ‼️‼️");
